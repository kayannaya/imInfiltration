...
# -----------------------------------------------------------------------------
# 11. Correlation with TIL abundance (ABIS / CD8+)
# -----------------------------------------------------------------------------
...
# Check how many barcodes actually overlap
cat("Overlapping barcodes:", 
    length(intersect(rownames(clean_BRCA_tpm_filtered_t), rownames(BRCA_TIL_abis))), "\n")
...
# -----------------------------------------------------------------------------
# 12. Correlation with immune markers (GSVA)
# -----------------------------------------------------------------------------

# NOTE: Place geneSets.csv at GENESETS_FILE (set above)
...
cat("til_vec NAs:", sum(is.na(til_vec)), "length:", length(til_vec), "\n")
cat("gene_vec NAs:", sum(is.na(gene_vec)), "length:", length(gene_vec), "\n")
cat("finite pairs:", sum(is.finite(til_vec) & is.finite(gene_vec)), "\n")
...
BRCA_expr_survival_coef <- read_excel(SURVIVAL_FILE, sheet = "BRCA")
...
# -----------------------------------------------------------------------------
# 14. Kaplan-Meier survival analysis
# -----------------------------------------------------------------------------
